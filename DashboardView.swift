...
class DashBoardView: UIScrollView,ChartDelegate,pieDelegate {
    ...
    func drawDashBoard(jsonData:NSData,_ filterDim:String?,_ filterVal:String?,_ inputX:[String], _ inputY:[String]) {
        ...
        let api = DataPersistance()
        print("\(api.getJsonData())")
        
        do {
//            let data = try NSJSONSerialization.dataWithJSONObject(api.getJsonData(), options: .PrettyPrinted);
//            let jsonString = NSString(data: data, encoding: NSUTF8StringEncoding);
            let jsonData: NSData = (api.getJsonData().dataUsingEncoding(NSUTF8StringEncoding))!;
    ...
    override func layoutSubviews() {
        ...
        self.contentSize  = CGSizeMake(width, 1.5 * height)
